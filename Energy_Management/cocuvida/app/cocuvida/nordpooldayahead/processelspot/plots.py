...
async def plot_date(elspot_data) -> str:
    ...
    for index in value_index:
        price = int(elspot_data['prices'][index]['value'])
...
async def plot_axvline_mark(elspot_data) -> str:
    ...
    for index in value_index:
        ...
        bars[index].set_width(0.7)
        bars[index].set_color(use_colour)
        time_start = elspot_data['prices'][index]['time_start']
        h = int(time_start[:2])
        m = int(time_start[3:])
        if timeofday.is_before_time(h,m):
            if not vertical_line_inserted:
                # index-0.6 = this index, but we move the vertical line slightly backwards (0.6) -> more accurate
