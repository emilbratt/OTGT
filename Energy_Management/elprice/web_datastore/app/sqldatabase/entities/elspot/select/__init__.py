...
class Select:
    ...
    def reshaped_v1(self, the_region: str, the_date: str)-> object:
    ...
    def raw_v1(self, the_date: str)-> object:
        cursor = self.cnxn.cursor()
        cursor.execute('''
            SELECT the_json FROM elspot_raw_v1
            WHERE the_date = :the_date
            ''', [the_date])
        res = cursor.fetchone()
        self.cnxn.close()
    ...
    def raw_exists_v1(self, the_date: str)-> bool:
        cursor = self.cnxn.cursor()
        cursor.execute('''
            SELECT COUNT(the_json) FROM elspot_raw_v1
            WHERE the_date = :the_date
            ''', [the_date])
        res = cursor.fetchone()[0]
        return (res > 0)
