...
class Page {
  ...
  function __construct () {
    ...
    echo Template::doc_title_left('Rapport: Utsolgte varer denne uken');
    ...
    foreach ($this->cnxn->query($this->query) as $row) {
      ...
      $day = CharacterConvert::utf_to_norwegian($row['Dag']);
  ...
  private function get_query_exlude_common () {
    ...
    SELECT
      ...
      DATENAME(weekday, articleStock.lastSold) AS Dag,
    ...
    WHERE
      DATEPART(WEEK, articleStock.lastSold) = DATEPART(WEEK, CURRENT_TIMESTAMP) AND
